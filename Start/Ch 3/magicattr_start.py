...
class Book:
    ...
    def __getattribute__(self, name):
        if name == "price":
            # you must user super() function to get price and discount attribute
            # otherwise, the function would go into infinite loop and program would crash
            p = super().__getattribute__("price")
            d = super().__getattribute__("_discount")
            return p - (p * d)
        # for all other attributes, just return them
        return super().__getattribute__(name)

    # TODO: __setattr__ called when an attribute value is set. Don't set the attr
    # directly here otherwise a recursive loop causes a crash
    def __setattr__(self, name, value):
        # here we will require to pass price as an float object
        if name == "price":
